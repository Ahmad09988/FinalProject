...
<?php
// handle the Unbooking process when the Unbook button is clicked
if(isset($_POST['unbook_room'])) {
  $room_id = mysqli_real_escape_string($db, $_POST['room_id_to_unbook']);

  // update the room record in the database
  $sql = "UPDATE rooms SET booked='not_booked', user_name='' WHERE id=$room_id_to_unbook";
  $result = mysqli_query($db, $sql);

  if($result) {
    // display a success message
    echo "Room is Unbooked!";
  }
}
?>
...
<div class="row py-lg-5">
	...
	<h1 class="fw-light">Your Reserved Room/s!</h1>	
</div>
...
$query = $db->query("SELECT * FROM rooms WHERE user_name ='ahmadsalem'");
...
<?php
					...
					<form method="POST">
  <input type="hidden" name="room_id_to_unbook" value="<?php echo $row["id"]; ?>">
  <input type="hidden" name="user_name_to_unbook" value="$username" >
  <button type="submit" name="unbook_room">Unbook Room</button>
</form>
					<?php
					}
